...
/***/ (function(module, exports, __webpack_require__) {
	...
	var textures = [{
	  file: './heat_distortion/index/img/desert.jpg',
	  name: 'image',
	  align: { x: 0.5, y: 0.5},
	  scale: { x: 1, y: 1 }
	}, {
	  file: './heat_distortion/index/img/desert-maps.jpg',
	  name: 'maps',
	//   align: { x: 0.9, y: 0.05 },
	//   scale: { x: 0.2, y: 0.2 }
	  align: { x: 0.5, y: 0.5},
	//   scale: { x: 0.8, y: 0.8 }
	  ...
	  file: './heat_distortion/index/img/noise.png',
	  name: 'noise',
	  repeat: true,
	  fill: false
	}]; 
	...
	if (window.DeviceOrientationEvent) {
		window.addEventListener('deviceorientation', function(event) {
			if (isTouchDevice && projection) {
				const adjustedX = (-event.gamma || 0) / 80; // Increased divisor for less sensitivity
				const adjustedY = ((-event.beta || 0) + 9.8) / 80; // Increased divisor for less sensitivity
